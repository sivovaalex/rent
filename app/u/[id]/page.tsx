...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!user) return { title: 'Пользователь не найден — Аренда PRO' };

  const title = `${user.name} — профиль на Аренда PRO`;
  const description = `${user.name}: рейтинг ${user.rating.toFixed(1)}, ${user.items.length} объявлений. Аренда PRO — шеринг-платформа.`;

  return {
    title,
    description,
    openGraph: { title, description, type: 'profile', locale: 'ru_RU', siteName: 'Аренда PRO' },
  };
}

...
import { Star, Upload, Eye, EyeOff, Package, Zap, Camera, Shirt, Dumbbell, Hammer } from 'lucide-react';
...
export default function Catalog({ 
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case 'approved': return <Badge variant="success" className="text-xs">Опубликован</Badge>;
      case 'pending': return <Badge variant="warning" className="text-xs">На модерации</Badge>;
...
// Компонент для создания/редактирования лота
function NewItemModal({ isOpen, onClose, item, currentUser, onSubmit }) {
  ...
  useEffect(() => {
    if (item) {
      ...
      // Предпросмотр существующих фото
      setPhotoPreviews(item.photos?.map(url => ({ preview: url })));
    } else {
      // Сброс формы при создании нового лота
      setNewItem({
        category: 'stream',
        subcategory: '',
        title: '',

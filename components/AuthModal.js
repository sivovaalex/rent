...
export default function AuthModal({ 
  ...
  const [registerRole, setRegisterRole] = useState('renter');
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (success) {
        // Закрываем модалку ПОСЛЕ успешного выполнения onLogin
        onClose();
      }
    } catch (error) {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if (success) {
        // Закрываем модалку ПОСЛЕ успешного выполнения onRegister
        onClose();
      }
    } catch (error) {
